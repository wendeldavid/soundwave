...
public class Semantico implements Constants {
	...
	String nomeDoMundo, nomeDoModelo, nomeModeloAux, nomeDoMetodo, idDaPeca, idDoModelo, nomeDaCor;
	...
	public Semantico(Compiler compiler) {
		...
		/*
		 * estes comandos abaixo foram incluídos aqui para que se poça compilar um método sem alterar as ações semanticas no GALS
		 */
		pilha = new Pilha();
	...
	public void executeAction(int action, Token token) throws SemanticError {
		...
		switch (action) {
		/*
		 * Cria novo MetodosDoModelo. Zera nomeDoModelo. Zera metodosUsados. Zera pecasCriadas. Cria pilha vazia. Inclui Método Viva Marca que esta compilando um Modelo
		 */
		case 0:
			metodosDoModelo = new HashMap<String, Comando>();
			nomeDoModelo = null;
			...
			break;
		/*
		 * Cria novo modelosDoMundo Cria novo statusDosModelosDoMundo Zera modelos Executavel Zera comandos do mundo Cria pilha e coloca um laço no topo Marca que esta compilando um mundo Inicial o
		 * mundo executavel
		 */
		case 1:
			modelosDoMundo = new HashMap<String, ModeloExecutavel>();
			statusDosModelosDoMundo = new HashMap<String, Integer>();
			modelosCompilados = new HashMap<String, HashMap<String, Comando>>();
			...
			break;
		/*
		 * Retira o Método da Pilha Cria um Modelo com apenas o Metodo CRIA
		 */
		case 2:
			compiler.setModelo(new ModeloExecutavel(metodosDoModelo, ""));
			break;
		/*
		 * Seta o nomeDoModelo se ele for nulo se Não For compara com o token se for diferente dispara um erro.
		 */
		case 3:
			if (nomeDoModelo == null)
				nomeDoModelo = token.getLexeme();
			else if (!nomeDoModelo.equals(token.getLexeme()))
				throw new SemanticError("Nome do Modelo Errado", token.getPosition());
		...
		case 4:
			...
			break;
		/*
		 * Grava Token na variável idDoModelo Zera variável nomeDoModelo
		 */
		case 5:
		...
		case 12:
			...
			break;
		/*
		 * Verifica se o Metodo atual é Cria Verifica se Peca ainda não foi criada Cria Comando Cria Peca Marca Peca como criada Marca variável modeloSeleciona como false
		 */
		case 13:
			if (!nomeDoMetodo.equals(ModelMaker.CRIA))
				throw new SemanticError("Comando cria deve ser " + "escrito na Ação CRIA!", token.getPosition());
			int tipoDaPeca = Integer.parseInt(idDaPeca.substring(1));
			if (tipoDaPeca < 1 || tipoDaPeca > 7)
				throw new SemanticError("O Nome da Peça é inválido", token.getPosition());
			if (pecasCriadas[tipoDaPeca - 1])
				throw new SemanticError("Peça já criada!", token.getPosition());
			comando = new ComandoCriaPeca(tipoDaPeca, new float[] { x, y, z });
			pecasCriadas[tipoDaPeca - 1] = true;
			modeloSelecionado = false;
			break;
		/*
		 * Adciona Comando no Laço do Topo da Fila Zera Variáveis X Y Z
		 */
		case 14:
			if (compilandoMundo) {
				if (comandoMundo == null)
					throw new SemanticError("Comando nulo", token.getPosition());
		...
		case 15:
			...
			break;
		/*
		 * marca modeloSelecionado como false verifica se peça foi criada
		 */
		case 16:
			modeloSelecionado = false;
		...
		case 22:
			...
			break;
		/*
		 * Cria Comando Laço Coloca Comando laço no topo da pilha
		 */
		case 23:
			if (compilandoMundo)
				pilha.push(new ComandoLaco(new ArrayList<Comando>(), (int) x));
			else
				pilha.push(new ComandoLaco(new ArrayList<Comando>(), (int) x));
			break;
		/*
		...
		case 24:
			...
			break;
		/*
		 * cria comando faca coloca comando faca no array de métodos usados
		 */
		case 25:
			comando = new ComandoCall(getSelectedModel(), token.getLexeme(), false, 0);
			metodosUsados.add(token.getLexeme());
			break;
		/*
		 * Retira laço do topo da pilha e adciona no hashmap de metodos do modelo
		 */
		case 26:
			metodosDoModelo.put(nomeDoMetodo, pilha.pop());
			break;
		/*
		 * Verifica se os métodos usados em comandos faca foram todos declarados Cria comando para o metodo VIVA do Modelo. Finaliza Modelo Criando objeto da classe ModeloExecutavel e devolvido-a a
		 * classe que construi esta.
		 */
		case 27:
			for (String s : metodosUsados) {
				if (!metodosDoModelo.containsKey(s))
					throw new SemanticError("Metodo: " + s + " não foi" + "criado no modelo e foi usado em um comando " + "Faça");
			}
			metodosDoModelo.put(ModelMaker.VIVA, new ComandoViva());
			compiler.setModelo(new ModeloExecutavel(metodosDoModelo, nomeModeloAux));
			break;
		/*
		 * Verifica se id Dado ao modelo já foi usado. Verifica se o Modelo está na lista de modelos já usados, caso não esteja, carrega, compila e guarda o modelo. Cria um ModeloExecutável com uma
		 * nova Figura e os comandos do modelo e adiciona na lista de modelos do mundo. coloca o status do modelo como criado em espera Cria comando que chama método cria do modelo.
		 */
		case 28:
			// Verifica se id Dado ao modelo já foi usado
			if (modelosDoMundo.containsKey(idDoModelo))
				throw new SemanticError(idDoModelo + " já foi usado.");
			...
			if (!modelosCompilados.containsKey(nomeDoModelo))
				try {
					...
					throw new SemanticError("Modelo " + nomeDoModelo + " não pode ser compilado!\n" + "O seguinte erro ocorreu:\n" + ae.getMessage());
				}
			// Cria um ModeloExecutável
			ModeloExecutavel modExe = new ModeloExecutavel(modelosCompilados.get(nomeDoModelo), nomeDoModelo);
			...
			comandoMundo = new ComandoCallMundo(idDoModelo, ModelMaker.CRIA, false);
			break;
		/*
		 * Cria comando para mover a figura para posição inicial apartir de x, y e z.
		 */
		case 29:
			comandoMundo = new ComandoMoveParaMundo(idDoModelo, nomeDoModelo, x, y, z);
			break;
		/*
		 * Verifica se id do modelo foi criado. verifica se ele está parado. Cria comando Viva. marca o modelo em metodo viva
		 */
		case 30:
			verificaIdModelo();
			if (statusDosModelosDoMundo.get(idDoModelo).equals(2))
				throw new SemanticError("O modelos já está executando " + "uma ação VIVA");
			comandoMundo = new ComandoVivaMundo(idDoModelo, nomeDoMetodo);
			statusDosModelosDoMundo.put(idDoModelo, new Integer(2));
			break;
		/*
		 * Verifica se id do modelo foi criado verifica se ele esta em viva cria comando termina
		 */
		case 31:
			verificaIdModelo();
			if (!statusDosModelosDoMundo.get(idDoModelo).equals(2))
				throw new SemanticError("O modelos não está executando " + "uma ação VIVA");
			comandoMundo = new ComandoCallMundo(idDoModelo, ModelMaker.TERMINA, true);
			statusDosModelosDoMundo.put(idDoModelo, new Integer(1));
			break;
		/*
		 * Verifica se id do modelo foi criado cria comando apaga
		 */
		case 32:
			verificaIdModelo();
			comandoMundo = new ComandoCallMundo(idDoModelo, ModelMaker.APAGA, true);
		...
		case 33:
			...
			break;
		/*
		 * Verifica se id do modelo foi criado cria comando facaMundo
		 */
		case 34:
			verificaIdModelo();
			comandoMundo = new ComandoCallMundo(idDoModelo, nomeDoMetodo, emParalelo);
			break;
		/*
	...
	private void verificaPecaCriada(Token token) throws SemanticError {
		if (!pecasCriadas[Integer.parseInt(idDaPeca.substring(1)) - 1])
			throw new SemanticError("Peça não foi criada!", token.getPosition());
	}
